...
for scale in [2,8,16,64,128,256]:
    ...
    if scale != 256:
        x_train /= scale
        x_test /= scale
        x_train.astype("int").astype("float32")
        x_test.astype("int").astype("float32")
    else:
        x_train = np.where(x_train > 0.5, 1, 0)
        x_test = np.where(x_test > 0.5, 1, 0)
    path = 'weight_'+str(scale)+'/'
    if not os.path.exists(path):
        os.mkdir(path)
    checkpoint = ModelCheckpoint((path+'resnet18.{epoch:02d}-{val_acc:.2f}.hdf5'), verbose=1, monitor='val_acc',save_best_only=True, mode='auto')
    lr_reducer = ReduceLROnPlateau(factor=np.sqrt(0.1), cooldown=0, patience=5, min_lr=0.5e-6)
    early_stopper = EarlyStopping(min_delta=0.001, patience=10)
    csv_logger = CSVLogger(path+'resnet18_PPG.csv')
    model = resnet.ResnetBuilder.build_resnet_18((3, img_rows, img_cols), nb_classes)
